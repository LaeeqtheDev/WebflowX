                ...
                page: (
                    await Promise.all(
                        results.page.map(async(message) =>{
                            ...
                            const dudupedReactions = reactionsWithCount.reduce(
                                (acc, reaction) => {
                                    ...
                                    if(existingReaction){
                                        ...
                                        acc.push({
                                            ...reaction, memberIds: [reaction.memberId]
                                        })
                                    }
                                    return acc;
                                },
                            ...
                            const reactionsWithoutMemberIdProperty = dudupedReactions.map(
                                ({memberId, ...rest}) => rest
                            )
                            return {
                                ...message,
...
export const getById = query({
    ...
    handler: async(ctx, args) => {
        ...
        const curretnMember = await getMember(ctx, message.workspaceId, userId)
